...
import se.sojo.apps.maths.calculator.core.Calculator.Companion.HISTORY_ITEM_TEXT_SIZE
...
import se.sojo.apps.maths.calculator.core.CustomAdapter
...
import se.sojo.apps.maths.calculator.core.hideSystemBars
import se.sojo.apps.maths.calculator.core.margin
import se.sojo.apps.maths.calculator.core.physicalScreenRectDp
import se.sojo.apps.maths.calculator.ui.addHistory
import se.sojo.apps.maths.calculator.ui.getHistoryItem
...
class MainActivity : ComponentActivity() {
    companion object { const val DEBUG = false }
    ...
    var historyItemTextSize: Float = 24f
    var lvHistoryResult: ListView? = null

    var tvHistoryListItem: TextView? = null

    var historyItemToAdd: String = ""

    var tvPreviousResult: TextView? = null
    var tvMemoryStatus: TextView? = null
    var tvDivider: View? = null

    var historyItems: ArrayList<String> = ArrayList()

    var historyArrayAdapter: CustomAdapter? = null
    ...
    // Lifecycle two (after onCreate) -> onStart
    override fun onStart() {
        ...
        HISTORY_ITEM_TEXT_SIZE = historyItemTextSize

        NOT_A_NUMBER = getString(R.string.nan)
        ...
        if (sharedPreferences.getBoolean("show_about_and_instructions", true)) {
            showInfoDialog(
                this,
                getString(R.string.about_and_instructions_title),
                "${getString(R.string.app_name)} v${info.versionName}\n${getString(R.string.copyright)}\n\n${
                    getString(R.string.information_and_instructions)
                }"
            )
        }
    }

    // Lifecycle three (after onStart) -> onResume
    override fun onResume() {
        super.onResume()

        if (DEBUG) Log.d("SOJO Debug:", "onResume()")

        val sharedPreferences = getSharedPreferences("UserPreferences", MODE_PRIVATE)

        val history = sharedPreferences.getString("history", "")?.trimStart(';')?.trimEnd(';')

        if (history!!.isNotBlank()) {
            val items = history.split(";")
            items.forEach { text ->  addHistory(text) }
        }
    }

    // Lifecycle four (after onResume) -> onPause
    override fun onPause() {
        super.onPause()

        if (DEBUG) Log.d("SOJO Debug:", "onPause()")
        ...
        sharedPreferences.edit {
            ...
            historyItems.forEach { text ->
                Log.d("SOJO Debug:", text)
                history += ";$text"
            }

            history = history.trimStart(';').trimEnd(';')

            putString("history", history)
        }
    }

    // Lifecycle five (after onPause) -> onStop
    override fun onStop() {
        super.onStop()
        if (DEBUG) Log.d("SOJO Debug:", "onStop()")
    }

    // Lifecycle six (after onStop) -> onDestroy
    override fun onDestroy() {
        super.onDestroy()
        if (DEBUG) Log.d("SOJO Debug:", "onDestroy()")
    }

    // Lifecycle one (before onStart) -> onCreate
    @Suppress("DEPRECATION")
    @SuppressLint("ClickableViewAccessibility")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        if (DEBUG) Log.d("SOJO Debug:","onCreate()")
        ...
        DECIMAL_SEPARATOR =
            DecimalFormatSymbols.getInstance(getCurrentLocale(this)).decimalSeparator.toString()
        ...
        tvHistoryListItem = findViewById(R.id.tv_history_item)

        layoutResult = findViewById(R.id.Result)

        tvDivider = findViewById(R.id.tv_divider)

        historyArrayAdapter = CustomAdapter(this, historyItems) //ArrayAdapter<String>? = null

        lvHistoryResult = findViewById(R.id.lv_history_result)
        lvHistoryResult?.adapter = historyArrayAdapter

        lvHistoryResult?.onItemClickListener = AdapterView.OnItemClickListener { _, _, position, _ -> getHistoryItem(position) }

        lvHistoryResult?.onItemLongClickListener = AdapterView.OnItemLongClickListener { _, _, _, _ ->
        ...
        if(savedInstanceState != null) {
            ...
            val history = savedInstanceState.getStringArrayList("History")
        ...
        if (DEBUG) {
            Log.d(
                "SOJO Debug:",
                "orientation: " + (if (orientation == 1) "Portrait" else "Landscape") + "\n" +
    ...
    private fun setScreenValues(
        ...
        historyItemTextSize = historyResultTextSize
    ...
    fun showInfoDialog(context: Context, title: String, mess: String) {
        val sharedPreferences = getSharedPreferences("UserPreferences", MODE_PRIVATE)
        ...
        builder.setPositiveButton(getString(R.string.i_have_understand)) { _, _ ->  }

        builder.setNegativeButton(getString(R.string.don_t_show_this_again)) { dialog, _ ->
            editor.putBoolean("show_about_and_instructions", false)
            editor.apply()
            dialog.dismiss()
        }
